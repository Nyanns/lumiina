...
func (r *userRepository) baseProfileQuery() *gorm.DB {
	return r.db.Select("id, username, role, is_verified, display_name, bio, avatar_url, banner_url, location, website, social_links, created_at").
...
func (r *userRepository) GetProfileByID(id uint) (*model.User, error) {
	var user model.User
	err := r.baseProfileQuery().First(&user, id).Error
	if err == nil {
...
func (r *userRepository) GetProfileByIdentifier(identifier string) (*model.User, error) {
	...
	if hashid.IsAllDigits(clean) {
		if num, err := strconv.ParseUint(clean, 10, 64); err == nil && num > 0 {
			if err := r.baseProfileQuery().First(&user, uint(num)).Error; err == nil {
				r.populateFollowCounts(&user)
				return &user, nil
			}
		}
	...
	if err := r.baseProfileQuery().Where("LOWER(username) = LOWER(?)", clean).First(&user).Error; err == nil {
		r.populateFollowCounts(&user)
		return &user, nil
	}
	...
	if decodedID, err := hashid.Decode(clean); err == nil && decodedID > 0 {
		if err := r.baseProfileQuery().First(&user, decodedID).Error; err == nil {
			r.populateFollowCounts(&user)
			return &user, nil
		}
	}
